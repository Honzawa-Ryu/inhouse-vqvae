...
class Trainer:
    ...
    def train(self):
        ...
        for i in tqdm(range(self.epoch, self.max_epochs), desc="Epochs"):
            ...
            # --- 訓練フェーズ ---
            self.model.train()
            train_loss, train_emb_loss, train_recon_loss = 0, 0, 0
            ...
            # --- 評価フェーズ ---
            self.model.eval()
            test_loss, test_emb_loss, test_recon_loss = 0, 0, 0
            ...
            grid = torchvision.utils.make_grid(combined_images, nrow=10, normalize=True)
