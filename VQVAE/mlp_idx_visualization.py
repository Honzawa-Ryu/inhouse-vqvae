...
def main(cfg: DictConfig):
    ...
    run = wandb.init(entity=cfg.wandb.entity, project=cfg.wandb.project, name="get-mlp-usage")
    ...
    # --- モデル解析のループ ---
    mlp = MLP(**cfg.model).to(device)

    try:
        artifact = run.use_artifact('VQVAE-MLP:latest')
    ...
    with torch.no_grad():
        for class_name, data_loader in class_specific_loaders.items():
            ...
            save_dir = os.path.join(parent_dir, f'tgtaa_mlpv{cfg.model.vqvae_n_embeddings}')
